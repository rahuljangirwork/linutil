...
redirect_to_latest_pre_release() {
  ...
  if [ -n "$latest_release" ]; then
    url="https://github.com/rahuljangirwork/linutil/releases/download/$latest_release/linutil"
  else
    printf "%b\n" 'Unable to determine latest pre-release version.' >&2
    printf "%b\n" "Using latest Full Release"
    url="https://github.com/rahuljangirwork/linutil/releases/latest/download/linutil"
  fi
  addArch
  printf "%b\n" "Using URL: $url"
}

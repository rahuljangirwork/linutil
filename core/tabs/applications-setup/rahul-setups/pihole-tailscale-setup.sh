...
check_port_53() {
    ...
    if "$ESCALATION_TOOL" ss -lptn 'sport = :53' | grep -q "LISTEN"; then
        printf "%b\n" "${YELLOW}Port 53 is in use. Checking for systemd-resolved...${RC}"
        ...
        if "$ESCALATION_TOOL" systemctl is-active systemd-resolved >/dev/null 2>&1; then
            ...
            printf "%b\n" "${RED}Something other than systemd-resolved is using Port 53. Please check manually: sudo ss -lptn 'sport = :53'${RC}"
            exit 1
        fi
    else
